...
- [Bug hunting 00: Потерянные регистры](#bug-hunting-00-потерянные-регистры)
  ...
  - [Исправление ошибки и проверка корректности работы](#исправление-ошибки-и-проверка-корректности-работы)
  - [Выводы](#выводы)
...
Заранее ткроем тестовую программу:
...
И адресам инструкций в памяти


Процессорное ядро в ходе корректного выполнения программы должно записать в регистр 3 значение 1, а после записать в память по адресу 64 значение 1 и оказаться в бесконечном цикле (разбор - в [соответствующем разделе практического занятия](../../practice/00_basic_hex/README.md#написание-тестовой-программы)).

На временной диаграмме не наблюдаем записи 1 в регистр 3, а в память по адресу 64 вместо 1 записывается значение 5.
...
А вот и "виновник торжества"! Адрес второго операнда в регистровом файле в RISC-V инструкции `beq` кодируется битами с 24 по 20:

![](../../doc/pic/riscv_instr_2.png)

Классическая bit-slice **ошибка локализована!** Переходим к исправлению.

## Исправление ошибки и проверка корректности работы

TODO: Поправить + перезапустить


## Выводы


[^1]: Большинство современный процессорных ядер имеет логику [предсказания условных переходов](https://en.wikipedia.org/wiki/Branch_predictor), повышающую их производительность. [Отличная статья по данной теме](https://blog.cloudflare.com/branch-predictor).