...
В ходе занятия анализируются доступные подходы к верификации, применяетя [подход "Hello world!"](../../theory/03_func.md#hello-world), делается вывод о некорректности исполнения программы процессорным ядром, [анализируются лог-файлы, временные диаграммы и RTL-описание](#поиск-ошибки). Ошибка локализуется, исправляется. Затем [проверяется корректность работы процессорного ядра](#проверка-корректности-работы).
...
На временной диаграмме не наблюдаем записи значения 1 в регистр 3, а в память по адресу 64 вместо значение 1 записывается значение 5. Ядро некорретно выполняет программу.
