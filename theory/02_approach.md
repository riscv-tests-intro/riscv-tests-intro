...
Применительно к процессору это может выражаться, например, в том, что он поддерживает только инструкции сложения `add` и вычитания `sub`, при этом, пока процессорное ядро не сгенерирует сигнал об успешном приеме инструкции `instr_ready`, ее значение `instr` на входе изменять нельзя.

Временная диаграмма корректной генерации тестовых воздействий в данном случае представлена на временной диаграмме ниже.

![](../doc/pic/riscv_wave_1.svg)
...
**Стоит заметить**, что, чтобы гарантировать отсутствие ошибок в устройстве, необходимо перебрать все возможные комбинации входных сигналов и все их допустимые последовательности, что в большинстве случаев не является возможным в виду размеров дизайна, его сложности и жестких временных рамок, которые диктует индустрия. **В современных реалиях задача верификатора – это, прежде всего, исключенин критических ошибок в аппаратуре.**
...
- ограничения не выходные сигналы определяют, каким должен быть результат работы устройства.
